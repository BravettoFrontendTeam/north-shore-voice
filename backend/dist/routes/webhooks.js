...
router.post('/call/incoming', async (req, res) => {
    try {
        ...
        if (isTwilio && process.env.TWILIO_AUTH_TOKEN) {
            ...
            const isValid = verifyTwilioWebhook(url, req.body, signature, process.env.TWILIO_AUTH_TOKEN);
            if (!isValid) {
...
router.post('/call/status', async (req, res) => {
    try {
        ...
        if (callStatus === 'completed' && sessionId) {
            await call_handler_1.callHandler.endCall(sessionId);
        }
...
router.post('/call/transcription', async (req, res) => {
    try {
        ...
        if (isFinal && sessionId && text) {
            await call_handler_1.callHandler.processSpeech(sessionId, text);
        }
        res.json({ success: true });
    }
    catch (error) {
