...
const elevenlabs_1 = __importDefault(require("./tts/elevenlabs"));
const cache_1 = __importDefault(require("./tts/cache"));
class AbëVoiceIntegration {
    ...
    async generate(options) {
        ...
        const cacheKey = cache_1.default.buildCacheKey({ text, voice: voiceId, provider: options.provider || 'abevoice', emotion: options.emotion, intensity: options.intensity, pacing: options.pacing });
        // Try cache first
        const cached = cache_1.default.getCachedAudio(cacheKey);
        if (cached) {
            return {
                success: true,
                audio_base64: cached.audio_base64,
                duration: undefined,
                metadata: { ...(cached.metadata || {}), provider: options.provider || 'cache' },
            };
        }
        // Provider override: delegate to ElevenLabs if requested
        if (options.provider === 'elevenlabs' || options.provider === 'eleven') {
            const el = new elevenlabs_1.default(process.env.ELEVENLABS_API_KEY);
            const res = await el.generate(options);
            if (res.success && res.audio_base64) {
                try {
                    cache_1.default.setCachedAudio(cacheKey, res.audio_base64, res.metadata || {});
                }
                catch (e) { }
            }
            return res;
        ...
        try {
            const response = await (0, node_fetch_1.default)(`${this.baseUrl}/api/v1/text-to-speech`, {
                ...
                body: JSON.stringify({
                    ...
                    metadata: {
                        ...
                        ...options.voice_style ? { voice_style: options.voice_style } : {},
            ...
            if (data.success && data.audio_base64) {
                try {
                    cache_1.default.setCachedAudio(cacheKey, data.audio_base64, data.metadata || {});
                }
                catch (e) { }
                try {
                    require('../utils/metrics').emitMetric('tts.request.latency', Date.now() - startTs, { provider: 'abevoice' });
                }
                catch (e) { }
                return {
                    success: true,
            ...
            try {
                require('../utils/failure-store').recordFailure('abevoice', 'request_failed', error instanceof Error ? error.message : String(error));
            }
            catch (e) { }
