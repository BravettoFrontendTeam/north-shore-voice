...
const trainingSamples = new Map<string, {
  id: string;
  filename: string;
  originalName: string;
  duration?: number;
...
const voiceModels = new Map<string, {
  id: string;
  name: string;
 ...
 */
router.delete('/samples/:sampleId', async (req: Request, res: Response) => {
  try {
 ...
 */
function simulateTraining(modelId: string) {
 ...
 */
router.put('/models/:modelId/personality', async (req: Request, res: Response) => {
  try {
    const { modelId } = req.params;
